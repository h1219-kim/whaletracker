...
```
- 집계 규칙: 회사별로 기간 내 공시들의 `delta.ratio`/`delta.shares` 합산(순변동). `parse_ok=false` 제외. `delta_ratio` 절대값 내림차순 상위 12개씩.
- 같은 회사의 exec/bulk 공시가 겹치면 둘 다 합산하되, **동일 rcp_no 중복 불가**. exec와 bulk가 같은 기간을 이중 보고할 수 있으나(드묾) 단순 합산으로 수용(한계로 문서화).
...
- exec/bulk 공시가 동일 변동을 이중 보고하는 드문 경우 합산 오차 가능.
