...
- **공개 사이트(주 경로)**: GitHub Actions가 **매일 18:40(KST) 스스로 데이터를 수집해
  사이트를 재배포**합니다. 내 PC와 무관하게 동작하며, KRX·DART·SEC 등 모든 소스를
  GitHub 서버에서 직접 가져옵니다(2026-07-14 KRX 포함 자체 수집 확인). 즉 공개
  사이트를 최신으로 유지하는 데 로컬 작업은 **필요 없습니다**.
- **로컬 앱 자동**: 로컬 서버가 켜져 있는 동안 15분마다 신선도를 점검해, 데이터가
  24시간보다 오래되면 다시 수집합니다(`--auto-refresh 6`으로 주기 변경, `0`으로 끄기).
  열려 있는 대시보드도 5분마다 새 데이터를 자동 반영합니다.
- **로컬 수동**: 대시보드의 **[데이터 갱신]** 버튼, 또는 CLI(로컬 `data/`만 갱신, push 안 함):

```powershell
.venv\Scripts\python.exe -m nps_fetcher --days 180 --max-new 300
# 또는 로그와 함께: scripts\refresh_task.cmd
```

수집은 이미 파싱한 공시를 재요청하지 않는 증분 방식이라 매일 돌아도 부담과 소요
시간이 작습니다(보통 1~2분).

> 참고: GitHub Actions는 사이트만 재배포하며 저장소의 `data/` 파일은 커밋하지 않습니다.
> 저장소의 `data/`는 GitHub 수집이 실패할 때 쓰이는 fallback 스냅샷입니다.
